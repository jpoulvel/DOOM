...
/*   Updated: 2020/02/18 15:18:38 by jpoulvel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_print_walls(t_fdf *img, t_wlist *wlst)
{
	if (wlst != NULL)
	{
		//ft_putendl("Ca entre dans wlst != NULL");//DEBUG
		while (wlst)
		{
		//	printf("walls printed :start (%f, %f)\nend(%f, %f)\n", wlst->wall.start.x, wlst->wall.start.y, wlst->wall.end.x, wlst->wall.end.y);//DEBUG
			ft_trace_line(wlst->wall.start, wlst->wall.end, img, RED);
